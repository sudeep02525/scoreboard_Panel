...
export default function MatchCard({ match }) {
  ...
  return (
    <Link href={`/matches/${match._id}`} className="block group">
      <div className="relative bg-gradient-to-br from-slate-900/90 to-slate-800/90 backdrop-blur-xl rounded-2xl border border-slate-700/50 p-5 shadow-xl hover:shadow-2xl hover:border-slate-600/50 transition-all duration-300 overflow-hidden">
        ...
        <div className="relative flex items-center justify-between mb-4">
          ...
          {isUpcoming && (
            <div className="px-3 py-1.5 rounded-full bg-blue-500/20 border border-blue-500/50">
        ...
        <div className="relative flex items-center justify-between gap-4 mb-4">
          ...
          <div className="flex-1">
            <h3 className="text-sm font-bold text-white mb-2 truncate">{match.teamA?.name}</h3>
            {match.innings1 ? (
              <div>
                <div className="flex items-baseline gap-1">
                  <span className="text-3xl font-black text-yellow-500">{match.innings1.runs}</span>
                  <span className="text-lg font-bold text-slate-400">/{match.innings1.wickets}</span>
                </div>
                <p className="text-xs text-slate-500 mt-1">({match.innings1.overs}.{match.innings1.balls} ov)</p>
              </div>
            ) : (
          ...
          <div className="flex-1 text-right">
            <h3 className="text-sm font-bold text-white mb-2 truncate">{match.teamB?.name}</h3>
            {match.innings2 ? (
              <div>
                <div className="flex items-baseline gap-1 justify-end">
                  <span className="text-3xl font-black text-yellow-500">{match.innings2.runs}</span>
                  <span className="text-lg font-bold text-slate-400">/{match.innings2.wickets}</span>
                </div>
                <p className="text-xs text-slate-500 mt-1">({match.innings2.overs}.{match.innings2.balls} ov)</p>
              </div>
            ) : (
              <div className="text-2xl font-black text-slate-700">-</div>
            )}
